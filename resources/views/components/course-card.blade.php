...
<article class="card flex flex-col">
    <img class="h-36 w-full object-cover" src=" @if($course->image){{ Storage::url($course->image->url)}} @endif" alt="">

    <div class="card-body flex-1 flex flex-col">
        <h1 class="card-title">{{ Str::limit($course->title, 40) }}</h1>
        <p class="text-gray-500 text-sm mb-2 mt-auto">Imparte: {{$course->teacher->name}}</p>
